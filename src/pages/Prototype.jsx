...
const PrototypePage = ({
  ...
  const getStylingAdvice = async () => {
    ...
    const likedDescriptions = likedItems
      ...
      .join("; ");
    const prompt = `Based on these clothing items I have liked: "${likedDescriptions}", suggest a few creative and fashionable outfit combinations. Provide the output in a 2-4 points on the basis of the items given.(Remove the numbers) keep it concise.`;
  ...
  const summarizeStyle = async () => {
    ...
    const prompt = `Based on this list of clothing items: "${likedNames}", what is my overall fashion style? Provide the output in a 2 sentences on the basis of the items given, keep it concise.`;
